        ...
        if (!$firstName || !$lastName || !$email || !$username) {
            ...
            if ($chk->num_rows > 0) {
                ...
                if (!$error) {
                    ...
                    $upd->bind_param('ssssssssssssssssi',
                        $firstName, $middleName, $lastName, $email, $username,
                        $phone, $dob, $gender, $bio,
...
:root {
  --ink:#0f0e0d;--ink-2:#3a3835;--ink-3:#7a776f;
  --paper:#f7f5f0;--paper-2:#eceae3;--paper-3:#e0ddd4;
  --gold:#c9972b;--gold-light:#f5e9cc;--gold-dark:#8a6318;
  --teal:#1b6b6b;--teal-lt:#d0eaea;
  --rose:#a02c2c;--rose-lt:#f5dede;
  --sky:#1a5d8f;--sky-lt:#d4e8f5;
  --radius:10px;--shadow:0 2px 16px rgba(15,14,13,.07);
}
...
.breadcrumbs{display:flex;align-items:center;gap:8px;font-size:12.5px;font-weight:500;letter-spacing:.04em;text-transform:uppercase;color:var(--ink-3);margin-bottom:2rem;}
.breadcrumbs a{color:var(--ink-3);text-decoration:none;transition:color .15s;}
.breadcrumbs a:hover{color:var(--gold);}
.breadcrumbs span{color:var(--paper-3);}
.breadcrumbs .cur{color:var(--ink-2);}

.page-header{margin-bottom:2rem;}
.page-header h1{font-family:'DM Serif Display',serif;font-size:clamp(2rem,4vw,3rem);font-weight:400;line-height:1;color:var(--ink);}
.page-header h1 em{font-style:italic;color:var(--gold);}
.page-header p{font-size:13.5px;color:var(--ink-3);margin-top:6px;}

.tab-nav{display:flex;gap:4px;margin-bottom:1.75rem;background:var(--paper-2);padding:5px;border-radius:var(--radius);width:fit-content;flex-wrap:wrap;}
.tab-btn{padding:8px 18px;border:none;background:transparent;border-radius:8px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;color:var(--ink-3);cursor:pointer;transition:all .2s;display:flex;align-items:center;gap:6px;}
.tab-btn.active{background:#fff;color:var(--ink);box-shadow:0 1px 6px rgba(15,14,13,.1);}
.tab-btn:hover:not(.active){color:var(--ink-2);}
.tab-panel{display:none;}
.tab-panel.active{display:block;}

.profile-avatar-wrap{display:flex;align-items:center;gap:1.75rem;margin-bottom:1.75rem;padding:1.5rem;background:#fff;border:1.5px solid var(--paper-3);border-radius:var(--radius);box-shadow:var(--shadow);}
.avatar-upload-zone{position:relative;flex-shrink:0;}
.avatar-circle{width:80px;height:80px;border-radius:50%;background:var(--gold-light);display:flex;align-items:center;justify-content:center;font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-dark);border:2px solid var(--paper-3);overflow:hidden;}
.avatar-circle img{width:100%;height:100%;object-fit:cover;}
.avatar-edit-btn{position:absolute;bottom:0;right:0;width:26px;height:26px;border-radius:50%;background:var(--gold);border:2px solid #fff;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:11px;transition:background .15s;}
.avatar-edit-btn:hover{background:var(--gold-dark);}
#avatarInput{display:none;}
.avatar-info h3{font-size:1.15rem;font-weight:600;color:var(--ink);}
.avatar-info p{font-size:13px;color:var(--ink-3);margin-top:3px;}
.avatar-meta{display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;}
.role-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;font-size:11.5px;font-weight:600;background:var(--gold-light);color:var(--gold-dark);}
.meta-chip{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;font-size:11.5px;font-weight:500;background:var(--paper-2);color:var(--ink-3);}

.stat-strip{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:2.25rem;}
.stat-card{background:var(--paper-2);border:1.5px solid var(--paper-3);border-radius:var(--radius);padding:1.1rem 1.25rem;position:relative;overflow:hidden;}
.stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--gold);}
.stat-card .s-val{font-family:'DM Serif Display',serif;font-size:2rem;line-height:1;color:var(--ink);}
.stat-card .s-lbl{font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-3);margin-bottom:8px;}

.profile-section{background:#fff;border:1.5px solid var(--paper-3);border-radius:var(--radius);padding:1.75rem;box-shadow:var(--shadow);margin-bottom:1.5rem;position:relative;overflow:hidden;}
.profile-section::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--gold);}
.profile-section.teal::before{background:var(--teal);}
.profile-section.sky::before{background:var(--sky);}
.profile-section h3{font-family:'DM Serif Display',serif;font-size:1.1rem;font-weight:400;color:var(--ink);margin-bottom:1.25rem;padding-bottom:10px;border-bottom:1.5px solid var(--paper-2);display:flex;align-items:center;gap:8px;}
.section-icon{width:28px;height:28px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;font-size:13px;}
.section-icon.gold{background:var(--gold-light);color:var(--gold-dark);}
.section-icon.teal{background:var(--teal-lt);color:var(--teal);}
.section-icon.sky{background:var(--sky-lt);color:var(--sky);}
...
.pf-field label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--ink-3);}
.pf-field input,.pf-field select,.pf-field textarea{background:var(--paper-2);border:1.5px solid var(--paper-3);border-radius:var(--radius);padding:9px 13px;font-family:'DM Sans',sans-serif;font-size:13.5px;color:var(--ink);outline:none;width:100%;transition:border-color .15s,background .15s;}
.pf-field input:focus,.pf-field select:focus,.pf-field textarea:focus{border-color:var(--gold);background:#fff;}
.pf-field input[readonly]{background:var(--paper-3);color:var(--ink-3);cursor:not-allowed;}
.pf-field textarea{resize:vertical;min-height:80px;}
.pf-field .hint{font-size:11.5px;color:var(--ink-3);margin-top:3px;}

.pf-field input.phone-error{border-color:var(--rose) !important;background:#fff;}
.pf-field input.phone-ok{border-color:var(--teal) !important;background:#fff;}
.phone-msg{font-size:11.5px;margin-top:3px;display:none;}
.phone-msg.show{display:block;}
.phone-msg.error{color:var(--rose);}
.phone-msg.ok{color:var(--teal);}

.pw-strength{height:4px;border-radius:2px;margin-top:6px;background:var(--paper-3);overflow:hidden;}
.pw-strength-bar{height:100%;border-radius:2px;width:0%;transition:width .3s,background .3s;}
.pw-show-toggle{position:relative;}
.pw-show-toggle input{padding-right:38px;}
.pw-eye{position:absolute;right:11px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--ink-3);cursor:pointer;font-size:14px;padding:2px;}

.toggle-list{display:flex;flex-direction:column;gap:14px;}
.toggle-item{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:var(--paper-2);border-radius:8px;border:1.5px solid var(--paper-3);}
.toggle-item-info h4{font-size:13.5px;font-weight:500;color:var(--ink);}
.toggle-item-info p{font-size:12px;color:var(--ink-3);margin-top:2px;}
.toggle-switch{position:relative;width:42px;height:24px;flex-shrink:0;}
.toggle-switch input{opacity:0;width:0;height:0;}
.toggle-slider{position:absolute;cursor:pointer;inset:0;background:var(--paper-3);border-radius:24px;transition:.3s;}
...
.info-item{background:var(--paper-2);border-radius:8px;padding:12px 14px;}
.info-item .i-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--ink-3);margin-bottom:4px;}
.info-item .i-val{font-size:13.5px;color:var(--ink);font-weight:500;}
.info-item .i-val.mono{font-family:monospace;font-size:12.5px;letter-spacing:.04em;}
.status-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#2e8b57;margin-right:5px;vertical-align:middle;}

.btn-save{display:inline-flex;align-items:center;gap:6px;background:var(--gold);color:#fff;border:none;border-radius:var(--radius);padding:9px 20px;font-family:'DM Sans',sans-serif;font-size:13.5px;font-weight:500;cursor:pointer;transition:background .2s,opacity .2s;}
.btn-save:hover{background:var(--gold-dark);}
.btn-save.teal{background:var(--teal);}
.btn-save.teal:hover{background:#0f4a4a;}
.btn-save.sky{background:var(--sky);}
.btn-save.sky:hover{background:#0f3d63;}
.btn-save.saved{background:var(--teal) !important;pointer-events:none;}
.btn-save:disabled{opacity:.8;cursor:not-allowed;pointer-events:none;}

.alert{padding:10px 16px;border-radius:var(--radius);font-size:13.5px;margin-bottom:1.5rem;display:flex;align-items:center;gap:8px;}
.alert-success{background:var(--teal-lt);color:var(--teal);border:1.5px solid var(--teal);}
.alert-error{background:var(--rose-lt);color:var(--rose);border:1.5px solid var(--rose);}

@media(max-width:680px){
  .pf-row,.pf-row-3,.info-grid{grid-template-columns:1fr;}
  .profile-avatar-wrap{flex-direction:column;text-align:center;}
...
phoneInput.addEventListener('input', function () { showPhoneState(validatePhone(this.value)); });
phoneInput.addEventListener('blur',  function () { showPhoneState(validatePhone(this.value)); });
phoneInput.closest('form').addEventListener('submit', function (e) {
  var valid = validatePhone(phoneInput.value);
  showPhoneState(valid);
  if (valid === false) { e.preventDefault(); phoneInput.focus(); }
});

function checkStrength(val) {
  var bar  = document.getElementById('pwBar');
  var lbl  = document.getElementById('pwLabel');
  var reqs = { len: val.length >= 8, upper: /[A-Z]/.test(val), num: /[0-9]/.test(val), special: /[^A-Za-z0-9]/.test(val) };
  ...
  var map = {len:'req-len',upper:'req-upper',num:'req-num',special:'req-special'};
  for (var k in map) {
    var el = document.getElementById(map[k]);
    if (reqs[k]) { el.style.color = 'var(--teal)'; el.textContent = el.textContent.replace('○','✓'); }
    else         { el.style.color = '';             el.textContent = el.textContent.replace('✓','○'); }
  }
}
