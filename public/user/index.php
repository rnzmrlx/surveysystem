    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        nav.topbar {
            ...
            padding: 0 2.5rem;
            display: flex;
            justify-content: flex-end; /* Push profile to right */
            align-items: center;
            background: #fff;
        ...
        .user-trigger {
            display: flex; align-items: center; gap: 12px;
            padding: 6px 12px; border-radius: 10px; transition: 0.2s;
        }
        .user-trigger:hover { background: var(--paper); }

        .avatar {
            width: 38px; height: 38px; background: var(--paper-2);
            border: 1.5px solid var(--paper-3); border-radius: 50%;
            display: flex; align-items: center; justify-content: center;
            font-weight: 600; color: var(--gold); font-size: 12px;
        }
        ...
        .dropdown-item {
            padding: 12px 16px; font-size: 14px; text-decoration: none;
            color: var(--ink); display: flex; align-items: center; gap: 10px;
        }
        .dropdown-item:hover { background: var(--paper); }
        .dropdown-item.logout { color: var(--fail); border-top: 1px solid var(--paper-3); }

        /* --- DASHBOARD CONTENT --- */
        .container { max-width: 1000px; margin: 3rem auto; padding: 0 2rem; }
        
        header h1 { font-family: 'DM Serif Display', serif; font-size: 2.8rem; font-weight: 400; margin-bottom: 0.5rem; }
        header h1 em { color: var(--gold); font-style: italic; }
        header p { color: var(--ink-3); margin-bottom: 3rem; }

        .section-label { font-size: 11px; font-weight: 700; text-transform: uppercase; color: var(--ink-3); margin-bottom: 1.5rem; border-bottom: 1px solid var(--paper-3); padding-bottom: 8px; }

        .survey-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem; margin-bottom: 4rem; }
        
        .card { background: #fff; border: 1.5px solid var(--paper-3); border-radius: var(--radius); padding: 1.5rem; transition: 0.2s; display: flex; flex-direction: column; }
        .card:hover { transform: translateY(-3px); border-color: var(--gold); box-shadow: 0 5px 15px rgba(0,0,0,0.03); }

        .btn-primary { background: var(--ink); color: var(--paper); text-decoration: none; padding: 10px; border-radius: 10px; text-align: center; font-size: 14px; font-weight: 600; margin-top: 1rem; }
        
        table { width: 100%; border-collapse: collapse; background: #fff; border: 1.5px solid var(--paper-3); border-radius: var(--radius); overflow: hidden; }
        th { text-align: left; padding: 12px 20px; background: var(--paper-2); font-size: 11px; color: var(--ink-3); text-transform: uppercase; }
        td { padding: 15px 20px; border-bottom: 1px solid var(--paper-2); font-size: 14px; }
    </style>
</head>
<body>

    <aside id="sidebar">
        <a href="index.php" class="sidebar-brand">Quick<em>Query</em></a>
        ...
        <nav class="topbar">
            <div class="user-dropdown" id="userDropdown">
                ...
                <div class="dropdown-menu" id="dropdownMenu">
                    ...
                    <a href="../../app/actions/logout.php" class="dropdown-item logout" onclick="return confirm('Logout?')">
