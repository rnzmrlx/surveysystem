...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .brand p { font-size: 13px; color: var(--ink-3); margin-top: 6px; }
    ...
    .toggle-pw svg { width: 15px; height: 15px; stroke: currentColor; fill: none; stroke-width: 2; }
    ...
    .terms-row label {
      font-size: 13px; color: var(--ink-3); cursor: pointer; line-height: 1.5;
    }
    .terms-row label a { color: var(--gold); text-decoration: none; font-weight: 600; }
    .terms-row label a:hover { color: var(--gold-dark); }
    ...
    .btn-register:hover { background: var(--ink-2); }
    ...
    .login-link a:hover { color: var(--gold-dark); }
    ...
    @media (max-width: 520px) {
      .card { padding: 1.75rem 1.25rem 2rem; }
...
<body>
      ...
      <form id="registerForm" action="/surveysystem/app/controllers/loginController.php" method="post" enctype="multipart/form-data" autocomplete="off" novalidate>
        ...
        <div class="form-row col-1">
          <div class="field">
            ...
            <div class="input-wrap">
              <svg class="icon" viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
  ...
  <script>
    ...
    document.getElementById('registerForm').addEventListener('submit', function(e) {
      ...
      const required = [
        { name: 'firstName',  errId: 'err-firstName' },
        { name: 'lastName',   errId: 'err-lastName' },
        { name: 'emailAddress', errId: 'err-email' },
        { name: 'username',   errId: 'err-username' },
        { name: 'password',   errId: 'err-password' },
        { name: 'street',     errId: 'err-street' },
        { name: 'barangay',   errId: 'err-barangay' },
        { name: 'city',       errId: 'err-city' },
      ];

      required.forEach(({ name, errId }) => {
